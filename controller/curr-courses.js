...
module.exports.delEval = function(req, res, next){
  ...
  currentCourse.findOne({user_id}, 'courses', (err, result)=>{
    ...
    let e_index = result.courses[c_index].evals.findIndex(eval => eval._id == eval_id);
    if(c_index < 0 || e_index < 0) return res.status(400).json({message: "Course or Evaluation Not Found"})
    let courses = result.courses;
...
module.exports.create = function (req, res, next){
  console.log("a post request was made to /api/curr-course, body is: ",req.body.currCourse);
  ...
  currentCourse.findOne({user_id}, (err, result)=>{
    if(!err && result){
      ...
      currentCourse.findOneAndUpdate({user_id}, newCourse, {upsert:true}, (err, n_Course)=>{
        if(err) return res.status(500).json(err);
...
module.exports.delete = function(req, res, next){
  ...
  currentCourse.findOne({user_id},'courses', (err, result)=>{
    ...
    if(c_index < 0) return res.status(400).json({message: "Course Not Found"})
