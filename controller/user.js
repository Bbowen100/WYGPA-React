...
module.exports.create = function(req, res, next){
  console.log("a post request was made to /api/user/signup, body is: ",req.body);
  let {user, password} = req.body;
  userModel.find({name: user}, (err0, results)=>{
    if(!results.length){
      bcrypt.hash(password, 10, function(err1, hash) {
        // Store hash in your password DB.
        userModel.findOneAndUpdate({name: user},{name: user, password: hash},{upsert:true},(err2, result)=>{
...
module.exports.receive = function(req, res, next){
  console.log("a post request was made to /api/user/login, body is: ",req.body);
  let {user, password} = req.body;
  userModel.findOne({name: user}, (err0, result)=>{
    if(result){
      bcrypt.compare(password, result.password, (err1, boolean)=>{
        if(err1){ return res.status(401).json({bool: null, error: err1}); }
        else{
          if(boolean){
